...
// typeCharacter types a single character using HID-level keyboard events
func typeCharacter(char rune) {
	...
	err = typeCharacterWithKeyboard(kb, char)
	if err != nil {
		fmt.Printf("Error typing character '%c': %v\n", char, err)
	}
}

// typeCharacterWithKeyboard types a single character using keyboard events
func typeCharacterWithKeyboard(kb keybd_event.KeyBonding, char rune) error {
	var keyCode int
	var needShift bool
	...
	default:
		...
		fmt.Printf("Unsupported character: '%c' (skipping)\n", char)
		return nil
	}
