...
func recordWavInterruptible(filename string, stopChan chan bool) error {
	...
	case "windows":
		...
		if _, err := exec.LookPath("ffmpeg"); err == nil {
			...
			cmd = exec.Command("powershell", "-ExecutionPolicy", "Bypass", "-File", scriptPath, "-OutputFile", filename)
		}
	default:
	...
	case <-stopChan:
		...
		if cmd.Process != nil {
			fmt.Println("Terminating recording process...")
			cmd.Process.Kill()
		}
		<-done // Wait for process to actually stop

		// For Windows PowerShell scripts, check if file was created
		if runtime.GOOS == "windows" {
			fmt.Println("Checking if PowerShell created the audio file...")
			time.Sleep(500 * time.Millisecond) // Brief wait for filesystem sync

			if _, fileErr := os.Stat(filename); fileErr == nil {
				if fileInfo, statErr := os.Stat(filename); statErr == nil && fileInfo.Size() > 0 {
					fmt.Printf("Audio file created successfully (size: %d bytes)\n", fileInfo.Size())
					return nil
				} else {
					fmt.Printf("File exists but has no content or stat error: %v\n", statErr)
				}
			} else {
	...
	case err := <-done:
		...
		if err != nil {
			// For Windows PowerShell scripts, check if the file was created successfully
			// PowerShell sometimes returns non-zero exit codes even when successful
			if runtime.GOOS == "windows" {
				fmt.Printf("PowerShell process returned error: %v, checking if file was created...\n", err)
				time.Sleep(500 * time.Millisecond) // Give filesystem time to sync
				if _, fileErr := os.Stat(filename); fileErr == nil {
					// Check if file has content (not empty)
					if fileInfo, statErr := os.Stat(filename); statErr == nil && fileInfo.Size() > 0 {
						fmt.Printf("Audio file created successfully (size: %d bytes), ignoring PowerShell exit code: %v\n", fileInfo.Size(), err)
						// File exists and has content, so recording was successful
						return nil
					} else {
						fmt.Printf("File exists but has no content or stat error: %v\n", statErr)
					}
